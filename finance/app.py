...
def index():
    ...
    # Calculate the current value of each stock holding
    total_value = cash
    for stock in stocks:
...
def buy():
    if request.method == "POST":
        ...
        # Deduct the purchase cost from the user's cash balance
        updated_cash = cash - total_cost
        db.execute("UPDATE users SET cash = ? WHERE id = ?", updated_cash, user_id)

        # Record the purchase transaction
        db.execute("""
            INSERT INTO transactions (user_id, symbol, shares, price, name)
            VALUES (?, ?, ?, ?, ?)
        """, user_id, symbol, shares, price, name)
...
def quote():
    ...
    if request.method == "POST":
        ...
        # Perform the lookup using the symbol
        quote_data = lookup(symbol)

        # Check if the symbol was valid and lookup successful
        if quote_data is None:
            return apology("Invalid stock symbol.")

        # Extract the relevant data from the quote_data
        symbol = quote_data["symbol"]
        name = quote_data["name"]
        price = quote_data["price"]

        # Render the quoted.html template with the retrieved data
